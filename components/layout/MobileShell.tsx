...
export function MobileShell({
  ...
  /** Omit for Staff roles — they have a single destination. */
  tabs?: NavItem[];

  /** Unread counts keyed by tab href. Plain data, safe across boundaries. */
    ...
    <div className="flex min-h-dvh flex-col bg-[var(--aurak-bg-subtle)]">
      ...
      <main
        className={cn(
          "min-w-0 flex-1",
          hasTabs &&
            "pb-[calc(var(--aurak-tabbar-h)+1rem)]"
        )}
      >
        {children}
      </main>
                ...
                >
                  <span className="relative">
                    ...
                    {badgeCount >
                      0 && (
                      <span
                        className="absolute -right-2 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--aurak-brand)] px-1 text-[0.5625rem] font-semibold leading-none text-white"
